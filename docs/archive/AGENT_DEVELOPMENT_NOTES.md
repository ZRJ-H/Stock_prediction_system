...
这个项目当前能做到 `pytest -q` 通过 87 项测试，关键在于模块边界比较清晰。

可测试性设计：

- Flask 路由和业务逻辑分离，`app.py` 只做请求响应。
- Agent 入口统一为 `agent.run(query, session_id, challenge_context)`。
- 工具通过 `run_tool(name, args)` 统一执行，并使用请求级上下文阻断盲测答案旁路。
...
- `.env` 配置是否按优先级加载。
- 工具和规则路由是否返回合理结果。
- 活动盲测期间聊天和工具层是否都能阻断行情泄漏。
- AI 情报是否严格使用目标日前数据且不输出涨跌答案。
...
- `pytest -q`：当前 87 passed，验证核心路由和边界。
...
9. 可测试工程：87 项 pytest 覆盖核心流程和边界。
10. MLP 可解释实验：真实日线、300维输入、训练报告和历史盲测公开展示泛化边界。
11. 安全配置：`.env` 本地加载，真实 API Key 不提交，前端不接收密钥。
12. Harness 化验收：固定演示链路 + 自动化测试 + mock + health check + diff check。
13. 盲测隔离：活动挑战期间仅开放通用投资知识，聊天入口和工具层双重防泄漏。
14. AI 情报助手：整理目标日前行情与资讯，但不直接输出涨跌方向。
